...
int main(int argc, char** argv) {
    ...
    // src in GPU (device)
    unsigned char* src_cuda;
    cudaMalloc(&src_cuda, sizeof(unsigned char)* height * width * channels);
    cudaMemcpy(src_cuda, src_img, sizeof(unsigned char)* height * width * channels, cudaMemcpyHostToDevice);
    
    // dst in GPU (device)
    unsigned char* dst_img = (unsigned char*)malloc(height * width * channels * sizeof(unsigned char));
    unsigned char* dst_cuda;
    cudaMalloc(&dst_cuda, sizeof(unsigned char)* height * width * channels);
    
    // 1204 threads in a block
    ...
    // passing dst_cuda to dst_img from device to host
    cudaMemcpy(dst_img, dst_cuda, sizeof(unsigned char)* height * width * channels, cudaMemcpyDeviceToHost);
